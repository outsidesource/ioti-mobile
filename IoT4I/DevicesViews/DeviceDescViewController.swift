//
//  DeviceDescViewController.swift
//  IoT4I
//
...
class DeviceDescViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        {
            self.title = "Device"
            self.lblName.text = self.device?.name
            self.lblLocation.text = self.device?.location
            self.lblDescription.text = self.device?.desc
            self.imgDevice.image = UIImage(named: self.device?.image ?? "")
            self.lblType.text = self.device?.deviceType
        }
