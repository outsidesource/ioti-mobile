...
class DeviceTableViewCell: UITableViewCell {
    ...
    var device:Device! {
        didSet {
            ...
            self.lblLocation.text = device.location
            self.lblActivation.text = "Activated: " + Utils.DDMMYY_HHMM(device.activation)
            self.imgEvent.image = UIImage(named: device.image ?? "defaultSensorIconBlue")
        }
    }
